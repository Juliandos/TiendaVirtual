...
router.put("/:id", async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Error al actualizar producto por ID" });
  }
});
...
LEFT JOIN (
    ...
    if (!sequelize) {
      throw new Error(
        "No es posible obtener la referencia al objeto Sequelize"
      );
    }
    ...
    return res
      .status(500)
      .json({ error: "Error al obtener información del producto" });
  }
});
